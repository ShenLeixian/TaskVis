...
def get_attributemap_for_NLP(data_value):
    if not isinstance(data_value, pd.DataFrame):
        return
    porter_stemmer_instance = PorterStemmer()
    columns=list(data_value.columns)
    attribute_map = dict()
    for attr in columns:
        ...
        attribute_map[attr]['stemmed_lower'] = ' '.join(porter_stemmer_instance.stem(t) for t in word_tokenize(attr_lower))
...
def compute_similarity(str1, str2, type):
    if type == 'cosine_similarity':
        # print("{} 和 {} 的cosine_similarity为{}".format(str1,str2,cosine_similarity(vectorize(str1), vectorize(str2.lower()))))
        return cosine_similarity(vectorize(str1), vectorize(str2.lower()))

    elif type == 'token_similarity':
        # value=fuzzy_token_similarity(str1, str2.lower())
        # if value>=0:
        #     print("{} 和 {} 的token_similarity为{}".format(str1,str2,fuzzy_token_similarity(str1, str2.lower())))
        # return value
        return fuzzy_token_similarity(str1, str2.lower())

    elif type == 'partial_ratio':
        # print("{} 和 {} 的partial_ratio为{}".format(str1,str2,fuzzy_partial_ratio(str1, str2.lower())))
        return fuzzy_partial_ratio(str1, str2.lower())

    elif type == 'ratio_similarity':
        # print("{} 和 {} 的ratio_similarity为{}".format(str1,str2,fuzzy_ratio_similarity(str1, str2.lower())))
        return fuzzy_ratio_similarity(str1, str2.lower())

    elif type == 'levenshtein_distance':
        # print("{} 和 {} 的levenshtein_distance为{}".format(str1,str2,levenshtein_distance(str1, str2.lower())))
...
def isfloat(datum):
    try:
        if datum == '' or str(datum).isspace() or datum is None:
            return False
        float(datum)
